    ...
    addNode(node) {
        ...
        if (this.graph.nodes[node.name]) {
            ...
            if (this.graph.nodes[node.name].type !== node.type) {
                throw new Error(
                    `Multiple projects are named "${node.name}". One is of type "${node.type
                    }" and the other is of type "${this.graph.nodes[node.name].type
...
module.exports = { ProjectGraphBuilder }