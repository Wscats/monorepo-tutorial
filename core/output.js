...
class CLIOutput {
    ...
    writeOutputTitle({ color, title, }) {
        this.writeToStdOut(` ${this.applyNxPrefix(color, title)}${os.EOL}`);
    }
    writeOptionalOutputBody(bodyLines) {
        ...
        this.addNewline();
        bodyLines.forEach((bodyLine) => this.writeToStdOut(`   ${bodyLine}${os.EOL}`));
    }
    ...
    addVerticalSeparatorWithoutNewLines(color = 'gray') {
        this.writeToStdOut(`${this.X_PADDING}${chalk.dim[color](this.VERTICAL_SEPARATOR)}${os.EOL}`);
    }
    error({ title, slug, bodyLines }) {
        ...
        if (slug && typeof slug === 'string') {
            this.addNewline();
            this.writeToStdOut(`${chalk.grey('  Learn more about this error: ')}https://errors.nx.dev/${slug}${os.EOL}`);
        }
        this.addNewline();
    }
...
module.exports = { output: new CLIOutput() }
